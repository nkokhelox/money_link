...
class PortraitPersonAmountsWidget extends StatelessWidget {
  final Person person;
  ...
  Widget getPerson() {
    ...
    final paidAmountTiles = amounts
        .where((amounts) => amounts.paidDate != null)
        .map((e) => EntityTile.amountTile(e))
        .toList();
    final unpaidAmountTiles = amounts
        .where((amounts) => amounts.paidDate == null)
        .map((e) => EntityTile.amountTile(e))
        .toList();

    final paidGroupTile = GroupTile(
        title: "SETTLED",
        subtitle: "Paid up amounts",
        innerTiles: paidAmountTiles);
    ...
    List<Widget> children = comboList
        .map((tile) => buildAmountTile(tile, 2 * titleLeftPad))
        .toList();
  ...
  Widget buildAmountTile(Tile tile, double titleLeftPad) {
    if (tile is GroupTile) {
      return Card(
        child: ExpansionTile(
          ...
          tilePadding: EdgeInsets.only(left: titleLeftPad),
          title: const Text("PAID AMOUNTS",
              style: TextStyle(fontWeight: FontWeight.bold, letterSpacing: 2)),
          subtitle: const Text("Money back to the owner",
              maxLines: 1, style: TextStyle(color: Colors.blueGrey)),
          onExpansionChanged: (_) => onTappedPerson(person),
          children: tile.innerTiles
              .map((t) => buildAmountTile(t, titleLeftPad * 2))
              .toList(),
        ),
      );
    }
    ...
    return AmountWidget(
        amount: (tile as EntityTile<Amount>).object,
        titleLeftPad: titleLeftPad);
  }
