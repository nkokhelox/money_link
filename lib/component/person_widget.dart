...
class PersonExpandableWidget extends StatelessWidget {
  final Person person;
  final bool isSelected;
  final double titleLeftPad;
  final void Function(Person) onTappedPerson;
  final void Function(Person) onPersonDeleted;
  const PersonExpandableWidget(
      {Key? key,
      required this.person,
      required this.isSelected,
      required this.titleLeftPad,
  ...
  void addAmount(BuildContext context, Person person) async {
    showBottomSheet(elevation: 3, context: context, enableDrag: true, builder: (context) => AddAmountForm(person: person));
  }
}
